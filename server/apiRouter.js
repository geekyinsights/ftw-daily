...
// STORE THIS IN A SAFER PLACE
const STORE_HASH = '8l7i9q0z6s';
const BIG_COMMERCE_AUTH_HEADERS = {
  'X-Auth-Token': '88deb3l5ft31k0aiwssay339x40hcor',
  'X-Auth-Client': 'npcix9k0ydub5a7zg1rf0633a65tk6c',
};
/**
...
router.get('/products', async (req, res) => {
  try {
    ...
    const { data } = await axios.get(url, { headers: BIG_COMMERCE_AUTH_HEADERS });
...
router.post('/newCustomer', async (req, res) => {
  const url = `https://api.bigcommerce.com/stores/${STORE_HASH}/v2/customers`;

  try {
    const { data } = await axios.post(url, { headers: BIG_COMMERCE_AUTH_HEADERS, body: req.body });
    console.log('data', data);
